...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (ids.length > 0) {
          const lastId = ids.sort(
            (a, b) => (parsed[b].lastUpdated || 0) - (parsed[a].lastUpdated || 0)
          )[0];
  ...
  const handleNewChat = useCallback(() => {
    ...
    const updated = { ...chats, [id]: newChat };
    saveChats(updated);
    setActiveChatId(id);
  ...
  const handleRenameChat = useCallback(
    (id: string, title: string) => {
      if (chats[id]) {
        const updated = { ...chats, [id]: { ...chats[id], title } };
        saveChats(updated);
      }
    },
    [chats, saveChats]
  ...
  const handleRegenerate = useCallback(
    (messageIndex: number) => {
      ...
      const updated = {
        ...chats,
        [activeChatId]: { ...chat, messages: newMessages },
      };
      saveChats(updated);
      setTimeout(() => handleSendAI(newMessages), 100);
    },
    [activeChatId, chats, isGenerating, saveChats]
  );
  ...
  // --- تحويل chats object إلى مصفوفة للـ Sidebar ---
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#212121] dark:bg-[#212121] light:bg-white">
      <ChatSidebar
        chats={chatsList}
      ...
      <main className="flex flex-1 flex-col overflow-hidden">
        {/* شريط علوي */}
        <header className="flex items-center justify-between border-b border-gray-700 px-4 py-3 dark:border-gray-700 light:border-gray-200">
          <select
            value={model}
            onChange={(e) => setModel(e.target.value)}
            className="rounded-lg border border-gray-600 bg-gray-800 px-3 py-1.5 text-sm font-medium text-white focus:outline-none dark:border-gray-600 dark:bg-gray-800 light:border-gray-300 light:bg-gray-100 light:text-gray-900"
          >
        ...
        <ChatWindow
          messages={
            isTyping
              ? [...activeMessages, { role: 'assistant' as const, content: typingText }]
              : activeMessages
          }
        ...
        {/* منطقة الإدخال */}
        <div className="border-t border-gray-700 px-4 py-3 dark:border-gray-700 light:border-gray-200">
          <div className="mx-auto flex max-w-3xl items-end gap-3 rounded-2xl border border-gray-600 bg-[#2f2f2f] px-4 py-3 dark:border-gray-600 dark:bg-[#2f2f2f] light:border-gray-300 light:bg-gray-100">
            <textarea
              ref={inputRef}
              id="message-input"
              value={inputValue}
              ...
              className="flex-1 resize-none bg-transparent text-sm text-white placeholder-gray-400 outline-none dark:text-white light:text-gray-900"
            />
